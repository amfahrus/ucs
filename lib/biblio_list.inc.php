...
class biblio_list
{
    ...
    protected function makeOutput()
    {
        ...
        while ($_biblio_d = $this->resultset->fetch_assoc()) {
            $_biblio_d['pdf'] = $_biblio_d['title'];
            $_biblio_d['lok'] = $sysconf['node'][$_biblio_d['node_id']]['id'];
	    $_biblio_d['nm'] = $sysconf['node'][$_biblio_d['node_id']]['name'];
            $text_uri = strtolower(preg_replace($old_pattern, $new_pattern , $_biblio_d['title']));
            //$_biblio_d['title'] = '<a href="'.$sysconf['baseurl'].'index.php?p=show_detail&id='.$_biblio_d['biblio_id'].'&'.urlencode(substr($text_uri,0,50)).'" title="'.__('Record Detail').'">'.$_biblio_d['title'].'</a>';
            $_biblio_d['title'] = '<a href="'.$sysconf['baseurl'].'show_detail-'.$_biblio_d['biblio_id'].'-'.urlencode(substr($text_uri,0,50)).'.html" title="'.__('Record Detail').'">'.$_biblio_d['title'].'</a>';
            // label
            ...
            // button
            $_biblio_d['detail_button'] = '<a href="'.$sysconf['baseurl'].'show_detail-'.$_biblio_d['biblio_id'].'-'.urlencode(substr($text_uri,0,50)).'.html" class="detailLink" title="'.__('Record Detail').'">'.__('Record Detail').'</a>';
            if ($this->xml_detail) {
            ...
            $_biblio_d['pdf_button'] = '<form action="http://www.primurlib.net" id="cse-search-box">
    <input type="hidden" name="cx" value="partner-pub-5549337631353789:y7lunfyw93f" />
    <input type="hidden" name="cof" value="FORID:10" />
    <input type="hidden" name="ie" value="ISO-8859-1" />
    <input type="hidden" name="q" value="'.$_biblio_d['pdf'].';filetype:pdf" />
    <input type="submit" name="sa" value="Search PDF" class="pdflink" />
</form><script type="text/javascript" src="http://www.google.com/cse/brand?form=cse-search-box&amp;lang=en"></script>';
$_biblio_d['lok_button'] = '<a href="#" title="Lihat lokasi di peta" onclick="openHTMLpop(\'lokasi.php?lok='.base64_encode($_biblio_d['lok']).'&nm='.base64_encode($_biblio_d['nm']).'\', 640, 480, \'Lokasi '.$_biblio_d['nm'].'\')" class="detailLink">Peta Lokasi</a>';
//$_biblio_d['psn_button'] = '<a href="#" title="Pesan buku" onclick="openHTMLpop(\'formmail.php?j='.base64_encode($_biblio_d['pdf']).'\', 640, 500, \'Pemesanan <br>'.$_biblio_d['pdf'].'\')" class="xmlDetailLink">Pesan Buku</a>';
...
// cover images var
            ...
            $_buffer .= '<div class="customField locationField">'.$_biblio_d['pdf_button'].'</div>';
            $_buffer .= '<div class="subItem">'.$_biblio_d['detail_button'].' '.$_biblio_d['xml_button'].' '.$_biblio_d['lok_button'].' '.$_biblio_d['psn_button'].'</div>';
            $_buffer .= "</div>\n";
            $_i++;
        }
