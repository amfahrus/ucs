...
class detail extends content_list
{
    ...
    protected function htmlOutput()
    {
        ...
        $this->record_detail['pdf'] = $this->record_detail['title'];
        ...
        while ($data = $_biblio_authors_q->fetch_row()) {
            $authors .= '<a href="'.$sysconf['baseurl'].'?author='.urlencode('"'.$data[0].'"').'&search=Search" title="'.__('Click to view others documents with this author').'">'.$data[0]."</a><br />";
        ...
        while ($data = $_biblio_topics_q->fetch_row()) {
            $topics .= '<a href="'.$sysconf['baseurl'].'?subject='.urlencode('"'.$data[0].'"').'&search=Search" title="'.__('Click to view others documents with this subject').'">'.$data[0]."</a><br />";
            $this->metadata .= $data[0].'; ';
        }
        $this->metadata .= '" />';
        ...
        */
        ...
        if (isset($this->record_detail['node_id']) && isset($sysconf['node'][$this->record_detail['node_id']])) {
            ...
            $this->record_detail['lok'] = base64_encode($this->record_detail['node_id']);
	    $this->record_detail['nm'] = base64_encode($_node_name);
	    $this->record_detail['nm_lok'] = $_node_name;
